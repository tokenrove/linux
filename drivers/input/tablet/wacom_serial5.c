...
MODULE_LICENSE("GPL");
// module parameters for thumbwheel configuration
static int thumbwheel;
static int th_mode; // default to scroll mode
static int pos_delay = 800;
static int neg_delay = -800;
static int deadband;
static int thumbwheel_offset;
module_param(thumbwheel, int, 0444);
...
static void handle_model_response(struct wacom *wacom)
{
	...
	if (p) {
		int r = sscanf(p + 1, "%u.%u", &major_v, &minor_v);

		if (r != 2)
			dev_dbg(&wacom->dev->dev,
				"got a bad version string");
	}
	...
	default:
		dev_dbg(&wacom->dev->dev,
			"Didn't understand Wacom model string: \"%s\". Maybe you want the protocol IV driver instead of this one?\n",
			wacom->data);
				   ...
				   struct tool_state *state)
{
	...
	/* Reset everything, otherwise we lose the initial states when
	 * in-prox next time
	 */
...
static void handle_device_id_packet(char *data, struct tool_state *state)
{
	...
	/* Don't enable it here, yet. Let a packet with an actual
	 * valid position etc do it.
	 */
	state->proximity = 0;
...
static void handle_packet(struct wacom *wacom)
{
	...
	if ((data[0] & 0xfc) == 0xc0)
		...
		/* General pen packet or eraser packet or airbrush
		 * first packet airbrush second packet
		 */
		handle_general_stylus_packet(dev, data, state);
	} else if (((data[0] & 0xbe) == 0xa8) || ((data[0] & 0xbe) == 0xb0)) {
		/* 4D mouse 1st packet or Lens cursor packet or 2D
		 * mouse packet
		 */
